...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        decoder_embed_dim=768; decoder_depth=4; decoder_num_heads=6;  mlp_ratio=4.
        ...
        self.decoder_blocks_feature = nn.ModuleList([
            Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer)
            for i in range(2)])

        self.decoder_norm = norm_layer(decoder_embed_dim)
        self.decoder_pred_ = nn.Linear(decoder_embed_dim, patch_size**2 * in_chans, bias=True) # decoder to patch

        self.decoder_norm_feat = norm_layer(decoder_embed_dim)
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        x_feat = self.decoder_blocks_feature[0](x)
        x_feat = self.decoder_blocks_feature[1](x_feat)
        x_feat = self.decoder_norm_feat(x_feat)
        x_feat = x_feat[:, 1:, :]
    ...
    def forward(self, imgs, register_blk=-1):
        B = imgs.shape[0]
        x = self.patch_embed(imgs)

        # add pos embed w/o cls token
        x_full = x + self.pos_embed[:, 1:, :]

        # masking: length -> length * mask_ratio
        x, mask, ids_restore = self.random_masking(x_full, 0.75)
        ...
        pred, pred_feat = self.forward_decoder(latent, ids_restore)
        loss = self.forward_loss(imgs, pred, mask)

        return latent, loss, pred, mask, pred_feat
